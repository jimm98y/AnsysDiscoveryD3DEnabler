        ...
        static void Main(string[] args)
        {
            AppDomain.CurrentDomain.AssemblyLoad += (o, e) =>
            {
                if (e.LoadedAssembly.FullName.Contains("StrideWrapper"))
                {
                    ...
                    var cmdLineInstance = Assembly.LoadFrom(ResolvePath("UI Foundation.dll")).GetType("SpaceClaim.CommandLine").GetProperty("Instance", BindingFlags.Static | BindingFlags.Public).GetValue(null, null);
                ...
                else if (e.LoadedAssembly.FullName.Contains("Disco.CoreUI"))
                {
                    ...
                    var cmdLineArgs = Assembly.LoadFrom(ResolvePath("Disco.Application.dll")).GetType("Ansys.Disco.CommandLine.CommandLineArgs").GetMethod("Parse", BindingFlags.Static | BindingFlags.Public).Invoke(null, new object[] { args });
                    ...
                    var cmdLineInstance = Type.GetType("System.Windows.Application, PresentationFramework, Version=4.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35").GetProperty("Current", BindingFlags.Static | BindingFlags.Public).GetValue(null, null);
                    cmdLineInstance.GetType().BaseType.GetField("_commandLineArgs", BindingFlags.NonPublic | BindingFlags.Instance).SetValue(cmdLineInstance, cmdLineArgs);
                }
            };
            // use binding redirects from Discovery.exe.config
            var assemblyBinding = XDocument.Load(ResolvePath("Discovery.exe.config")).Root.Element("runtime").Elements(XName.Get("assemblyBinding", "urn:schemas-microsoft-com:asm.v1"));
            AppDomain.CurrentDomain.AssemblyResolve += (o, e) =>
            {
                ...
                if (dependentAssemblies == null) return null;
            ...
            Assembly.LoadFrom(ResolvePath("Discovery.exe")).GetType("Ansys.Disco.Program").GetMethod("Main").Invoke(null, new object[] { args });
        }
        private static string ResolvePath(string fileName) { return Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), fileName); }
    }
}
